...
//            let Z = self.utils.linkageAdjacentWard(data: aggregated, stopAtK: 8)
//            let labels = self.utils.fclusterFromPartialZ(Z: Z, n: aggregated.count) // ajuste t conforme desejado
//            DispatchQueue.main.async {
...
//    }
//    func exportToCSV() -> URL? {
//        // Fetch today's matrices and their backing objects
//        let todays = fetchFeaturesForToday()
//        guard !todays.isEmpty, !self.buffer.isEmpty else {
//            print("No data to export")
//            return nil
//        }
//
...
//        let fileURL = documentDirectory.appendingPathComponent(fileName)
//
//        // CSV header: timestamp, id, row_index, col_0, col_1, ...
//        var csvText = "timestamp,id,row_index"
//        if let firstMatrix = self.buffer.first, let firstRow = firstMatrix.first {
//            for colIndex in 0..<firstRow.count {
//                csvText.append(",col_\(colIndex)")
//            }
//        }
//        csvText.append("\n")
//
//        // Write each row of each reconstructed matrix
//        for (idx, fm) in todays.enumerated() {
//            let ts = fm.timestamp ?? ""
//            let idString = fm.id?.uuidString ?? ""
//            let matrix = idx < self.buffer.count ? self.buffer[idx] : []
//            for (rowIdx, row) in matrix.enumerated() {
//                var line = "\(ts),\(idString),\(rowIdx)"
//                for value in row {
//                    line.append(",\(value)")
//                }
//                line.append("\n")
...
//        deleteRequests(for: context)
//
//        // CRITICAL: Clear buffer after export to free memory
